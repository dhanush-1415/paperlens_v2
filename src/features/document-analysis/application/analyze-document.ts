  ...
  return async function analyzeDocument(input) {
    ...
    if (!input.media) {
      if (text.length < INPUT_LIMITS.minDocumentChars) {
        return err(
          validationError({
            text: [`Needs at least ${INPUT_LIMITS.minDocumentChars} characters.`],
          }),
        );
      }
    ...
    const flags = await deps.analyzer.analyze({
      text,
    ...
    const dateEntities = flags.value.entities?.filter(
      (e) =>
        e.iconHint === 'calendar' ||
        e.label.toLowerCase().includes('date') ||
        e.label.toLowerCase().includes('deadline'),
    );
