  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border-subtle bg-surface-1 shadow-sm">
      <div className="flex items-center justify-between border-b border-border-subtle bg-surface-2 px-3 py-2.5">
        ...
        <span className="text-[11px] font-bold tracking-widest text-text-primary uppercase">
      ...
      <div className="grid grid-cols-7 border-b border-border-subtle px-2 py-1.5">
        {DAY_ABBR.map((d) => (
          <div
            key={d}
            className="text-center text-[9px] font-bold tracking-widest text-text-tertiary uppercase"
          >
            {d}
          </div>
        ))}
              ...
              className={cn(
                ...
                isSelected
                  ...
                  : isDisabled
                    ...
                    : isToday
                      ? 'cursor-pointer font-semibold text-brand-primary ring-1 ring-brand-primary/40 ring-inset hover:bg-brand-primary/10'
...
export function ReminderButton({
  ...
  const visiblePresets = presets.filter(
    (p) => p.isDaily || p.isCadence || !selected.includes(p.key),
  );
  const dailyAllSelected = dailyKeys.length > 0 && dailyKeys.every((k) => selected.includes(k));
  const cadenceAllSelected =
    cadenceKeys.length > 0 && cadenceKeys.every((k) => selected.includes(k));
  ...
  const deadlineLabel = deadlineDate
    ? new Date(`${deadlineDate}T00:00:00`).toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric',
        year: 'numeric',
      })
    ...
    <>
      {variant === 'hero' ? (
        <button
          onClick={handleOpen}
          disabled={isPending}
          className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-brand-primary py-2.5 font-bold text-white shadow-[0_0_20px_rgba(var(--brand-primary),0.3)] transition-all hover:bg-brand-primary/90 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-70"
        >
        ...
        <button
          ...
          className={cn(
            'group flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl border border-border-subtle bg-surface-2 px-4 py-2.5 text-text-secondary transition-all duration-200 hover:border-border-strong hover:bg-surface-raised hover:text-text-primary hover:shadow-sm sm:w-auto',
          )}
        >
          <Bell className="h-4 w-4 text-text-tertiary transition-colors group-hover:text-text-primary" />
          <span className="hidden text-xs leading-none font-semibold sm:inline">Remind</span>
        </button>
      ) : (
        <button
          onClick={handleOpen}
          className={cn(
            'flex w-full cursor-pointer items-center gap-3 rounded-xl px-3.5 py-3',
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[12px] leading-tight font-semibold text-text-primary">
          ...
          <Bell className="h-4 w-4 shrink-0 animate-pulse text-brand-primary" />
        </button>
      )}
        ...
        createPortal(
          ...
          >
            ...
            >
              ...
              <div
                ref={scrollBodyRef}
                className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 pb-3"
              >
                ...
                {visiblePresets.length > 0 && (
                  <div>
                    <p className="mb-2.5 flex items-center gap-1.5 text-[10px] font-bold tracking-widest text-text-tertiary uppercase">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {visiblePresets.map((preset) => {
                        if (preset.isCadence) {
                          const canAddMore =
                            cadenceKeys.some((k) => !selected.includes(k)) && !atMax;
                          return (
                            <button
                              key="cadence"
                              type="button"
                ...
                {selected.length > 0 && (
                  <div>
                    <div className="mb-2 flex items-center justify-between">
                      <p className="text-[10px] font-bold tracking-widest text-text-tertiary uppercase">
                        Your reminders
                      </p>
                      <div className="flex items-center gap-1">
                        ...
                        <span className="ml-1 text-[10px] font-bold text-text-tertiary tabular-nums">
                          {selected.length}/{MAX}
                        </span>
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {selected.map((key) => (
                        <span
                          key={key}
                          className="flex items-center gap-1.5 rounded-full border border-brand-primary/25 bg-brand-primary/10 py-1 pr-1.5 pl-2.5 text-[11px] font-medium text-text-primary"
                        >
              ...
              <div className="shrink-0 space-y-2 border-t border-border-subtle px-5 py-4">
                ...
                >
                  {isPending ? (
                    ...
                    <>
                      <Bell className="h-4 w-4" />
                      Set {selected.length} Reminder{selected.length > 1 ? 's' : ''}
                    </>
                  )}
                </button>
                <button
