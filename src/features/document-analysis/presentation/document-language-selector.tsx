    ...
    <div ref={containerRef} className="relative shrink-0">
      <button
        ...
        className={cn(
          ...
          'cursor-pointer bg-surface-1 shadow-sm transition-all duration-150 select-none',
          currentLanguage
      ...
      >
        ...
        />
        <span className="max-w-[90px] truncate">{currentLang ? currentLang.name : 'Auto'}</span>
        <ChevronDown
          className={cn(
      ...
      {open && (
        <div
          className={cn(
            'absolute right-0 bottom-[calc(100%+6px)] z-50 sm:top-[calc(100%+6px)] sm:bottom-auto',
        ...
        >
          <div className="sticky top-0 z-10 border-b border-border-subtle/50 bg-surface-1/98 px-3 py-2.5 backdrop-blur-sm">
            <div className="flex items-center gap-2 rounded-xl border border-border-subtle bg-surface-2 px-2.5 py-1.5 transition-colors focus-within:border-brand-primary/30">
          ...
          <div className="custom-scrollbar max-h-[300px] overflow-y-auto py-1.5">
            {!query && (
              <div className="px-2 pb-1">
                ...
                >
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border border-border-subtle/50 bg-surface-2 text-base shadow-sm">
                    🌐
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="text-[12.5px] leading-none font-semibold">Auto</p>
              ...
              REGIONS.map((region) => {
                ...
                return (
                  <div key={region} className="px-2">
                    <p className="px-3 pt-2 pb-1 text-[10px] font-bold tracking-widest text-text-tertiary/70 uppercase">
                      {region}
                    </p>
                    {langs.map((lang) => (
                      <LangOption
          ...
          <div className="border-t border-border-subtle/50 bg-surface-2/50 px-4 py-2">
...
function LangOption({
  ...
  return (
    ...
    >
      <div className="min-w-0 flex-1">
        <p className={cn('text-[13px] leading-tight font-medium', selected && 'font-semibold')}>
          {lang.name}
        </p>
        <p className="mt-0.5 text-[10px] leading-none text-text-tertiary">{lang.nameEn}</p>
      </div>
