...
import { Copy, RefreshCw, Archive, Trash2, ShieldCheck, Scale, Globe, CheckCircle2, Sparkles, Headphones } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full w-full overflow-hidden bg-surface-1">
      ...
      <div className="flex-1 overflow-y-auto h-full block custom-scrollbar relative pb-10">
        ...
        {(analysis.mimeType?.startsWith('audio/') || analysis.mimeType?.startsWith('video/')) && analysis.rawText && (
          <div className="px-6 md:px-8 mt-4">
            <details className="group relative z-10 rounded-2xl border border-border-subtle bg-surface-2 px-5 py-4 shadow-sm transition-all">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-2 text-xs font-semibold text-text-secondary outline-none">
                ...
                <span className="rounded-full bg-brand-primary/10 text-brand-primary px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider group-open:hidden">View</span>
                <span className="rounded-full bg-surface-1 border border-border-strong px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider hidden group-open:inline text-text-secondary">Hide</span>
              </summary>
              <div className="mt-4 pt-4 border-t border-border-subtle/50 flex flex-col gap-3">
                <p className="text-sm text-text-primary leading-relaxed bg-surface-1 py-4 px-5 rounded-xl border border-border-subtle/50 max-h-[400px] overflow-y-auto whitespace-pre-wrap font-serif">
