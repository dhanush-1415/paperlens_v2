...
function buildFlatLines(node: Node, depth: number, parentKeys: string[], lines: FlatLine[]): void {
  ...
  lines.push({
    num: lines.length + 1,
    depth,
    kind: foldable ? 'open' : isVoid ? 'void' : 'void',
    tag,
    attrs,
    nodeKey,
    parentKeys: [...parentKeys],
...
export function CollapsibleHtmlViewer({
  ...
  return (
    <div
      ref={innerRef}
      className="min-h-0 flex-1 overflow-auto rounded-xl select-none"
      style={{ background: '#1e1e1e' }}
    >
      {/* Title bar */}
      <div
        className="sticky top-0 z-10 flex items-center gap-2 border-b px-4 py-2"
      ...
      >
        ...
        {/* Code lines */}
        <div
          style={{ flex: 1, paddingTop: 12, paddingBottom: 12, paddingLeft: 0, overflowX: 'auto' }}
        >
          {visible.map((line) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ paddingLeft: 8, userSelect: 'text' }}>
                  ...
                  {line.kind === 'text' && <span style={{ color: C.text }}>{line.text}</span>}

                  {(line.kind === 'open' || line.kind === 'void') && (
                    <>
                      <span style={{ color: C.tag }}>
                        {'<'}
                        {line.tag}
                      </span>
...
export function TextPreview({
  ...
  if (type === 'csv') {
    ...
    return (
      <div ref={innerRef} className="min-h-0 flex-1 overflow-auto p-4 pr-12">
          ...
          <thead>
            <tr>
              {headers.map((h, i) => (
                <th
                  key={i}
                  className="border border-border-subtle bg-surface-2 px-3 py-2 text-left font-semibold text-text-primary"
                >
                  {h.replace(/^"|"$/g, '')}
                </th>
              ))}
            ...
            {body.map((row, ri) => (
              <tr key={ri} className="even:bg-surface-2/50">
                {row.map((cell, ci) => (
                  <td
                    key={ci}
                    className="border border-border-subtle/50 px-3 py-1.5 text-text-secondary"
                  >
                    {cell.replace(/^"|"$/g, '')}
                  </td>
                ))}
  ...
  if (type === 'markdown') {
    return (
      <div ref={innerRef} className="min-h-0 flex-1 overflow-y-auto px-6 py-5 pr-12">
        <Markdown
          components={{
            h1: ({ children }) => (
              <h1 className="mt-6 mb-3 text-xl font-bold text-text-primary first:mt-0">
                {children}
              </h1>
            ),
            h2: ({ children }) => (
              <h2 className="mt-5 mb-2 text-lg font-bold text-text-primary">{children}</h2>
            ),
            h3: ({ children }) => (
              <h3 className="mt-4 mb-2 text-base font-semibold text-text-primary">{children}</h3>
            ),
            p: ({ children }) => (
            ...
            code: ({ children }) => (
              <code className="rounded bg-surface-2 px-1 py-0.5 font-mono text-xs text-text-primary">
                {children}
              </code>
            ),
            pre: ({ children }) => (
              <pre className="mb-3 overflow-x-auto rounded-lg bg-surface-2 p-4 font-mono text-xs text-text-primary">
                {children}
              </pre>
            ),
  ...
  // Plain text
  return (
    <div ref={innerRef} className="min-h-0 flex-1 overflow-y-auto p-5 pr-12">
      <pre className="font-mono text-xs leading-relaxed break-words whitespace-pre-wrap text-text-secondary">
        {content}
      </pre>
    </div>
  );
