  ...
  return (
    <section className="w-full py-24 relative overflow-hidden bg-canvas border-t border-border-strong/30">
      ...
      <div className="w-[95%] md:w-[90%] lg:w-[80%] mx-auto px-6 relative z-10 flex flex-col gap-16">
        ...
        <ScrollReveal variant="stagger-children" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {groups.map((group) => (
            <div 
              key={group.category} 
              className="reveal-item flex flex-col gap-5 rounded-3xl border border-border-strong/60 bg-surface-1 shadow-[0_4px_20px_rgb(0,0,0,0.03)] p-7 transition-all duration-500 hover:border-brand-primary/40 hover:-translate-y-1 hover:shadow-[0_12px_40px_-10px_rgba(var(--brand-primary-rgb),0.15)] group/card relative overflow-hidden"
            >
              {/* Premium Hover Blob */}
        ...
        <ScrollReveal variant="fade-up" delay={0.2}>
          <Button variant="premium" className="group/link shadow-2xl shadow-brand-primary/20" asChild>
            <Link href={ROUTES.useCases}>
              Browse all {groups.reduce((acc, g) => acc + g.guides.length, 0)} document guides
              <svg className="size-4 ml-2 transition-transform group-hover/link:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
