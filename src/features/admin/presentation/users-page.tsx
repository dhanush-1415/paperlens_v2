...
export function AdminUsersPage() {
  const columns: Column<(typeof USERS)[0]>[] = [
    ...
    },
    {
      ...
      header: 'Role',
      cell: (u) => (
        <Badge tone={u.role === 'admin' ? 'brand' : 'neutral'}>{u.role.toUpperCase()}</Badge>
      ),
    },
    {
      id: 'status',
    ...
    },
    {
      id: 'lastActive',
      header: 'Last Active',
      cell: (u) => <span className="text-text-secondary">{u.lastActive}</span>,
    },
    {
      id: 'actions',
      header: '',
      ...
      <Card className="flex flex-col overflow-hidden border-border-subtle shadow-card">
        <div className="flex items-center gap-4 border-b border-border-subtle bg-surface-2 p-4">
          <div className="relative w-full max-w-sm">
            ...
            <Input type="search" placeholder="Search users..." className="bg-surface-1 pl-9" />
          </div>
        </div>
