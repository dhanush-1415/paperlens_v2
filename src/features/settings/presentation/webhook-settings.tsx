  ...
  return (
    <div className="rounded-[1.25rem] border border-border-subtle bg-surface-1 p-5 px-6 shadow-sm">
      <div className="flex flex-col gap-4">
        {webhooks.length === 0 ? (
          <Text tone="secondary" size="sm">No webhooks configured.</Text>
        ) : (
          <div className="flex flex-col gap-2">
            {webhooks.map(wh => (
              <div key={wh.id} className="flex items-center justify-between p-3 rounded-lg border border-border-subtle bg-surface-2/50">
                <div className="flex items-center gap-3">
                  <WebhookIcon className="size-4 text-brand-primary" />
                  <div>
                    <Text className="font-semibold text-sm">{wh.url}</Text>
                    <Text size="xs" tone="secondary">Subscribed to: {wh.events.join(', ')}</Text>
                  </div>
                </div>
                <Button variant="outline" size="sm" onClick={() => handleDelete(wh.id)} className="h-8 w-8 p-0 text-red-500 hover:text-red-600 hover:bg-red-50">
        ...
        <div className="mt-4 border-t border-border-subtle pt-4 flex flex-col gap-3">
          <Text className="font-semibold text-sm">Add Webhook (B2B Integrations)</Text>
          <div className="flex flex-col sm:flex-row gap-3">
            <input 
              ...
              className="flex-1 rounded-lg border border-border-subtle bg-surface-1 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary"
            />
            <input 
              type="text" 
