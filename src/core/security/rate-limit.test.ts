...
describe('retry-after', () => {
  it('rounds up to whole seconds and never returns zero', () => {
    ...
    expect(
      retryAfterSeconds({ allowed: false, limit: 1, remaining: 0, resetAt: 1_500 }, 1_000),
    ).toBe(1);
    expect(
      retryAfterSeconds({ allowed: false, limit: 1, remaining: 0, resetAt: 4_100 }, 1_000),
    ).toBe(4);
  });

  it('returns at least one second for a window that already closed', () => {
    expect(retryAfterSeconds({ allowed: false, limit: 1, remaining: 0, resetAt: 0 }, 9_000)).toBe(
      1,
    );
  });
});
