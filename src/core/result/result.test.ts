...
describe('map / mapErr', () => {
  ...
  it('mapErr is the mirror image', () => {
    const mapped = mapErr(err(new AppError('INTERNAL_ERROR')), () =>
      notFoundError('document', 'x'),
    );
...
describe('unwrapping', () => {
  it('unwrapOr and unwrapOrElse supply a fallback only on failure', () => {
    ...
    expect(unwrapOrElse(err(new AppError('INTERNAL_ERROR')), (e) => e.code.length)).toBe(
      'INTERNAL_ERROR'.length,
    );
  });
