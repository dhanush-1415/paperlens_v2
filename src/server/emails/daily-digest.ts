...
const TH =
  'padding:10px 12px;text-align:left;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:#6b7280;border-bottom:2px solid #e5e7eb;white-space:nowrap;';
const TD =
  'padding:9px 12px;font-size:12px;color:#374151;border-bottom:1px solid #f3f4f6;vertical-align:top;';
...
<div style="max-width:900px;margin:32px auto;padding:0 16px;">
  ...
  try {
    ...
    if (error) console.error('[daily-digest] Resend error:', error);
      ...
      console.log(
        `[daily-digest] Sent — ${summary.total_events} events, ${summary.unique_users} users`,
      );
  } catch (err) {
