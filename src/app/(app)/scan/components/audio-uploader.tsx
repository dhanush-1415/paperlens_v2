...
import { Mic, Upload, AlertTriangle, Brain, FileText, Headphones, Volume2 } from 'lucide-react';
...
const IDLE_BARS = [
  4, 8, 6, 12, 9, 16, 10, 20, 13, 18, 8, 22, 14, 18, 10, 15, 7, 19, 11, 16, 8, 13, 6, 10, 5,
];
const PREVIEW_BARS = [
  6, 14, 9, 20, 15, 24, 18, 28, 22, 26, 16, 30, 20, 26, 14, 22, 10, 25, 16, 22, 12, 18, 9, 14, 7,
];
  ...
  return (
    ...
    >
      <div className="relative z-10 flex w-full max-w-sm flex-col items-center gap-5">
        ...
        <p className="mt-2 text-xs text-text-tertiary">Max 10 MB · Automatically transcribed</p>
      </div>
    </div>
  );
}
...
export function AudioUploader({ onFileReady, onScanningChange, disabled }: AudioUploaderProps) {
  ...
  return (
    <>
      ...
      {state === 'analyzing' && <AudioAnalyzingState phase={phase} />}

      {state === 'error' && (
        <AudioError message={error ?? 'Analysis failed. Please try again.'} onRetry={handleRetry} />
      )}
    </>
  );
}
