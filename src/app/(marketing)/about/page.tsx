...
export const metadata: Metadata = {
  title: 'About Us',
  description:
    'PaperLens was founded with a single belief: individuals and developers should not need a law degree or accounting background to understand their daily documents.',
  ...
  return (
    <>
      ...
      <section className="relative w-full overflow-hidden bg-canvas py-24">
        ...
        <div className="relative z-10 mx-auto flex w-[95%] max-w-4xl flex-col gap-8 md:w-[90%] lg:w-[70%]">
          {PRINCIPLES.map((principle) => (
            <div
              key={principle.title}
              className="group relative flex flex-col items-start gap-6 overflow-hidden rounded-3xl border border-border-strong bg-surface-1 p-8 shadow-card transition-all duration-500 hover:-translate-y-1 hover:border-brand-primary/40 hover:shadow-2xl sm:flex-row"
            >
              <div className="pointer-events-none absolute top-0 right-0 h-32 w-32 rounded-full bg-brand-primary/5 opacity-0 blur-[30px] transition-opacity duration-700 group-hover:opacity-100" />

              <div
                className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl shadow-sm ${principle.isSuccess ? 'border border-risk-safe/20 bg-risk-safe/10 text-risk-safe' : 'border border-brand-primary/20 bg-brand-primary/10 text-brand-primary'}`}
              >
                {principle.icon}
              </div>
