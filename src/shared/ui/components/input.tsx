...
export interface InputProps
  extends Omit<InputHTMLAttributes<HTMLInputElement>, 'size'>, ControlVariantProps {
  /**
...
export function Input({ className, size, startAdornment, endAdornment, ...props }: InputProps) {
  ...
  return (
    ...
    <span className="relative flex w-full items-center">
      ...
      {control}
      {endAdornment ? (
        <span className="absolute end-3 flex items-center text-text-tertiary">{endAdornment}</span>
      ) : null}
    </span>
  );
}
